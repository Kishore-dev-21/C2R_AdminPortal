...
import { nationalMetrics, products, stockByDistrict, stockTrend, districts, fraudCases, activityLogs } from "@/data/mockData";
import {
...
const SuperAdminDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-bold text-foreground">{t("super.title")}</h2>
        <p className="text-xs text-muted-foreground">{t("super.subtitle")} {new Date().toLocaleTimeString()}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="command-panel p-4">
          <h3 className="text-sm font-semibold text-foreground mb-4">{t("super.national_stock")}</h3>
          <div className="h-64">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }} className="command-panel p-4">
          <h3 className="text-sm font-semibold text-foreground mb-4">{t("super.district_stock")}</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="command-panel p-4">
        <h3 className="text-sm font-semibold text-foreground mb-4">{t("super.stock_trend")}</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }} className="command-panel">
          <div className="px-4 py-3 border-b border-border"><h3 className="text-sm font-semibold text-foreground">{t("super.district_status")}</h3></div>
          <div className="overflow-x-auto">
            <table className="data-table">
              <thead><tr><th>{t("common.district")}</th><th>{t("districts.shops")}</th><th>{t("common.stock")} (T)</th><th>{t("common.status")}</th></tr></thead>
              <tbody>
                {districts.map(d => (
                  <tr key={d.id}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="command-panel">
          <div className="px-4 py-3 border-b border-border"><h3 className="text-sm font-semibold text-foreground">{t("super.fraud_alerts")}</h3></div>
          <div className="overflow-x-auto">
            <table className="data-table">
              <thead><tr><th>{t("fraud.type")}</th><th>{t("common.district")}</th><th>{t("fraud.severity")}</th><th>{t("fraud.progress")}</th></tr></thead>
              <tbody>
                {fraudCases.map(f => (
                  <tr key={f.id}>
                    ...
                    <td>
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-primary" style={{ width: `${f.progress}%` }} /></div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }} className="command-panel">
        <div className="px-4 py-3 border-b border-border"><h3 className="text-sm font-semibold text-foreground">{t("super.recent_activity")}</h3></div>
        <div className="overflow-x-auto">
          <table className="data-table">
            <thead><tr><th>{t("logs.action")}</th><th>{t("logs.user")}</th><th>{t("logs.details")}</th><th>{t("logs.timestamp")}</th></tr></thead>
            <tbody>
              {activityLogs.slice(0, 5).map(l => (
                <tr key={l.id}><td className="font-medium">{l.action}</td><td>{l.user}</td><td className="text-xs text-muted-foreground">{l.details}</td><td className="text-xs">{l.timestamp}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
